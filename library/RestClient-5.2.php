...
class RestQuery
{
    public static function get ($id = null)
    {
        ...
        if (isset($class)) {
            $__class = new ReflectionClass($class);
            $mode = $__class->getStaticPropertyValue('__name');
            $params = call_user_func(array($class, 'getAttributes'));
            ...
            if ($params !== false)
            {
                ...
                $url = str_replace(" ", "", REST_SERVER . $mode . "?" . $param_url);
        ...
        if ($name != "__name") {
            ...
            if (isset($class)) {
                ...
                if (array_key_exists($name, $class_vars)) {
                    $__class->setStaticPropertyValue($name, $value);
                }
            }
        }
        return false;
    ...
    public static function getAttributes ()
    {
        ...
        if ($class) {
            ...
            foreach ($class_vars as $key => $var)
            {
                if (! empty($var))
                {
                    $return[$key] = $__class->getStaticPropertyValue($key);
                }
            }
    ...
    public static function getAttribute ($name)
    {
        if ($name != "__name") {
            ...
            if (isset($class)) {
                ...
                if (array_key_exists($name, $class_vars)) {
					return (( $__class->getStaticPropertyValue($name) ==NULL)?false: $__class->getStaticPropertyValue($name) );
                }
            }
        }
        return false;
    ...
    function get_called_class()
	{
		...
		foreach (debug_backtrace() as $bt) {
			...
			if (isset($bt['class']) && ($bt['type'] == '->' ||$bt['type'] == '::') && isset($bt['file']))
			{
				...
				if ($cache["{$file}_{$line}"] != 'parent' && $cache["{$file}_{$line}"] != 'self' && !empty($cache["{$file}_{$line}"]))
				{
